			...
			for i, ob_main in enumerate(objects):
				...
				for ob, ob_mat in obs:
					with ProgressReportSubstep(subprogress1, 6) as subprogress2:
						...
						# Vertex Colors
						has_vcol = me.vertex_colors
						if EXPORT_COLORS:
							if has_vcol:
								active_col_layer = me.vertex_colors["Col"] # me.vertex_colors.active
								if not active_col_layer:
									has_vcol = False
								#else:
								#	active_col_layer = active_col_layer.data
							if has_vcol:
								col = me.vertex_colors["Col"].data[:] #[:]
								# THIS DOESN'T WORK
								# COLOR ARRAY IS FOUND WHEN WRITING BELOW v
						...
						# Vert
						if EXPORT_COLORS and has_vcol :
							#ALESAN99
							#Write vertex colors (r,g,b) after coordinates
							vci = 0
							
							#create vertex to vertex color dictionary
							#(verticies are shared between polys, not colors, so there are more vertex colors than verticies)
							col_lookup = {}
							for f, f_index in face_index_pairs:
								for vi, (v_idx, l_idx) in enumerate(zip(f.vertices, f.loop_indices)):
									col_lookup[v_idx] = l_idx
							...
							for v in me_verts:
								...
								# FIND VERTEX COLOR
								# print(v.index)
								# print(col[v.index].color)
								# print(me.vertex_colors["Col"].data[v.index].color)
								color = (1,1,1)
								vci = col_lookup[v.index] #vertex color index
								if me.vertex_colors.active.data[vci] : #v.index
									col = me.vertex_colors.active.data[vci].color
